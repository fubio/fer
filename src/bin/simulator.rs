...
impl PCSampler {
    pub fn new(numPCExpriringArr: Vec<(u64, u64)>)-> PCSampler {
        PCSampler {
...
impl Sampler {
    pub(crate) fn new<T: Iterator<Item = (u64, f64)>>(t: T) -> Sampler {
...
fn expectation(vec: Vec<(u64, f64)>) -> f64 {
...
pub fn caching(ten_dist: Sampler, cache_size: u64, delta: f64) -> (f64, f64, f64) {
    ...
    let unstored = normalize_map(cache.FERemainingTracker).iter().map(|(key, value)| *key as f64 * *value).sum::<f64>();
    let fer = cache.force_evictions as f64/trace_len as f64;
